...
describe('POST /api/essentiality-levels', () => {
	...
	it('creates a level and returns 201', async () => {
		...
		const request = new NextRequest('http://localhost/api/essentiality-levels', {
			method: 'POST',
			body: JSON.stringify({ code: 'ESSENTIAL', label: 'Essential', sortOrder: 1 }),
	...
	it('returns 409 when code already exists', async () => {
		mockService.create.mockRejectedValue(new Error('EssentialityLevel code already exists'))
		const request = new NextRequest('http://localhost/api/essentiality-levels', {
			method: 'POST',
			body: JSON.stringify({ code: 'ESSENTIAL', label: 'Essential', sortOrder: 1 }),
			headers: { 'Content-Type': 'application/json' },
